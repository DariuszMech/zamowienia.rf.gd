...
<body>
	...
	<main>
			...
			<div class="contaiiner">
						...
						<?php	
							echo "<nav class='sticky'><a href='lista.php?id=$id' class='link'><div class='cofnij'>COFNIJ</div></a><a href='menu.php' class='link'><div class='cofnijmenu'>MENU</div></a></nav>";
						?>
						<header class="col-12">
...
<?php	
	...
	{		
		...
		$rezultat = mysqli_query($polaczenie, "SELECT koszyk.id ,koszyk.idzam, koszyk.idu ,grupa.idgrp from grupa, grupowicze, zamowienia, koszyk where zamowienia.idgrpzam='$id'  and zamowienia.idgrpzam = grupa.idgrp and grupowicze.idczlonkagr='{$_SESSION["id"]}' and grupowicze.idgr = grupa.idgrp and zamowienia.idzam = '$idzam' and koszyk.idu = grupowicze.idczlonkagr and koszyk.idzam=zamowienia.idzam ");
		
		$ile = mysqli_num_rows($rezultat);
		if($ile>=1)
		{	
			$_SESSION['kosz']='To zamówienie już znajduje się w twoim koszyku';			
		}
		else
		{	
	
				$rezultat = $polaczenie->query(" INSERT INTO `koszyk`(`idzam`, `idu`) VALUES ('$idzam', '{$_SESSION["id"]}' ) ");	
				if($rezultat) 
				{
					$_SESSION['powiadomienie']="Dodano zamówienie nr: $idzam do koszyka";
...
echo<<<END
			<label class="link"><div class="ddkoszyka">Dodaj do koszyka
			<form method="post">
...
END;
			...
			{
				echo "<div class='error'>".$_SESSION['kosz']."</div>";
				unset($_SESSION['kosz']);
			}
echo<<<END
			
			<a href='edytujzam.php?id=$id&idzam=$idzam' class='link'><div class='edytujzam'>Edytuj zamówienie</div></a>
